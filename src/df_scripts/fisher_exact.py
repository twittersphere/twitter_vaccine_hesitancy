...
def read_date_data(df_path, drop_id=True, remove_rest=True):
    us_ready = pd.read_parquet(f"{df_path}/us_id_date_state_filteredlabels.parquet")
...
def prepare_data(us_ready, world_ready, daily_us_ready, state_us_ready):
    ...
    return us_ready, world_ready, daily_us_ready, state_us_ready
...
def convert2df_and_save(ready_data, fisher_vals, state_date, value, file_path):
    ...
    df = pd.DataFrame({state_date: value,
...
def main():
    ...
    us_ready, world_ready, daily_us_ready, state_us_ready = prepare_data(
                                                                us_ready,
                                                                world_ready,
                                                                daily_us_ready)
    ...
    convert2df_and_save(world_ready, world_fisher_values, 'date', unique_dates,
